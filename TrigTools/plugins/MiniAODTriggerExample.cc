#ifndef CMSSWTools_TRIGTOOLS_MINIAODTRIGGEREXAMPLE
#define CMSSWTools_TRIGTOOLS_MINIAODTRIGGEREXAMPLE
...
{ 
  ...
  auto jetsHandle = getHandle(iEvent,jetsToken_);
  ...
  for(auto& pathName : pathsToPass_){
    ...
    if(pathIndex>=trigNames.size()) cout <<" path "<<pathName<<" not found in menu"<<endl;
      ...
      if(trigResultsHandle->accept(pathIndex)) cout <<" path "<<pathName<<" passed"<<endl;
      else cout <<" path "<<pathName<<" failed"<<endl;
    }
  }
  ...
  //
  std::vector<bool> filtersPassed;
  for(std::string filter : filtersToPass_){
    bool foundFilter = false;

    for(auto& trigObj : trigObjsUnpacked){
      foundFilter = foundFilter | trigObj.hasFilterLabel(filter);
    }
    filtersPassed.push_back(foundFilter);
    //cout << "\t " << filter << " was passed ? " << foundFilter << endl;
  }

  cout << " filters Passed: ";
  for(bool thisFilter : filtersPassed){
    cout << thisFilter << " ";
  }
  cout << endl;

  //cout <<"checking eles "<<endl;
